...
def search():
    if request.method == "POST":
       ...
       try:
           ...
           session["cityCountry"] = totalCityInfo["country"]
...
def city():
    ...
    infoCountry = session["cityCountry"]

    return render_template("index.html", Country = infoCountry, citynameTitle = infoCityName, cityPopulation = infoCityPopulation , cityTimezone = infoCityTimezone, long = infoCityLong, lat = infoCityLat, tempNow=infoTempNow, feelsLike = infoTempFeelsLike, min = infoMin, max = infoMax, humitity = infoHumidity )
